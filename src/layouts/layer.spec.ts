...
describe('pickNextLayerSpec', () => {
  it('uses dagre through 5, radial through 20, then cloud', () => {
    ...
    expect(pickNextLayerSpec(21, BANDS)).toEqual({ mode: 'cloud' });
    expect(pickNextLayerSpec(100, BANDS)).toEqual({ mode: 'cloud' });
  });

  it('treats a single spec as a catch-all band', () => {
    expect(pickNextLayerSpec(3, { mode: 'cloud' })).toEqual({ mode: 'cloud' });
    expect(pickNextLayerSpec(40, { mode: 'cloud' })).toEqual({ mode: 'cloud' });
  });
